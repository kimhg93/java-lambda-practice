...
/**
 * 중간(중개) 연산
 * 스트림의 중간 연산은 스트림의 요소를 변환하거나, 걸러내는 등의 작업을 수행한다.
 * 중간 연산은 스트림을 리턴하며, 여러 개의 중간 연산을 연결하여 사용할 수 있다. 중간 연산은 최종 연산이 호출되기 전까지는 실제로 수행되지 않는다.
 * 중간 연산은 스트림 파이프라인을 구성하여 데이터를 가공하는데 유용하게 사용된다.
 *
 * 주요 중간 연산
